...
class ControllerExtensionModuleOCFilter extends Controller {
  ...
  public function copy($data = []) {
    ...
    $this->ocfilter->admin->setControllerEntityConfig(true);

    $json = [];

    $this->load->language('extension/module/ocfilter/setting');

    // Opencart Global Setting
    if ($this->ocfilter->opencart->version >= 30) {
      $prefix = 'module_ocfilter';
    } else {
      $prefix = 'ocfilter';
    }
    ...
    if ($this->user->hasPermission('modify', 'extension/module/ocfilter') && $this->user->hasPermission('modify', 'extension/module/ocfilter/filter')) {
      ...
      $setting = [
        'copy_type' => $this->ocfilter->admin->getEntityValue('copy_type', 'checkbox'),
        'copy_dropdown' => $this->ocfilter->admin->getEntityValue('copy_dropdown'),
        'copy_status' => $this->ocfilter->admin->getEntityValue('copy_status', 1),
        'copy_attribute' => $this->ocfilter->admin->getEntityValue('copy_attribute', 1),
        'copy_attribute_id' => $this->ocfilter->admin->getEntityValue('copy_attribute_id', []),
        'copy_attribute_id_exclude' => $this->ocfilter->admin->getEntityValue('copy_attribute_id_exclude', 1),
        'copy_attribute_group_id' => $this->ocfilter->admin->getEntityValue('copy_attribute_group_id', []),
        'copy_attribute_group_id_exclude' => $this->ocfilter->admin->getEntityValue('copy_attribute_group_id_exclude', 1),
        'copy_attribute_category_id' => $this->ocfilter->admin->getEntityValue('copy_attribute_category_id', []),
        'copy_attribute_category_id_exclude' => $this->ocfilter->admin->getEntityValue('copy_attribute_category_id_exclude', 1),
        'copy_group_as_attribute' => $this->ocfilter->admin->getEntityValue('copy_group_as_attribute'),
        'copy_filter' => $this->ocfilter->admin->getEntityValue('copy_filter'),
        'copy_option' => $this->ocfilter->admin->getEntityValue('copy_option'),
        'copy_option_in_stock' => $this->ocfilter->admin->getEntityValue('copy_option_in_stock', 1),
        'copy_value_separator' => $this->ocfilter->admin->getEntityValue('copy_value_separator', []),
        'copy_truncate' => $this->ocfilter->admin->getEntityValue('copy_truncate'),
      ...
      if (!empty($this->request->post['copy_save_setting'])) {
        foreach ($setting as $key => $value) {
          $this->ocfilter->setting->add($key, $value);
        }
      }

      // For js finish triggering at log
      if (isset($this->request->get['copy_hash'])) {
        $setting['copy_hash'] = $this->request->get['copy_hash'];
      }

      $this->model_extension_module_ocfilter_filter->copyFilters($setting);
  ...
  protected function checkInstall() {
    ...
    // DB
    if (!$this->model_extension_module_ocfilter_setting->isColumnExists('ocfilter_filter_to_category', 'status')) {
      $this->db->query("ALTER TABLE `" . DB_PREFIX . "ocfilter_filter_to_category` ADD `status` TINYINT UNSIGNED NOT NULL DEFAULT 0 AFTER `category_id`");
    }
