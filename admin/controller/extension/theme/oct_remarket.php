...
class ControllerExtensionThemeOCTReMarket extends Controller {
	...
	public function index() {
		...
		if ($this->config->get('theme_oct_remarket_data_bought_together_cron')) {
            ...
            $data['bought_together_cron_url'] = $site_link . "index.php?route=octemplates/events/helper/octMassGenerateBoughtTogether&cron_pass=" . urlencode($this->config->get('theme_oct_remarket_data_bought_together_cron'));
        } else {
    ...
    private function generateCss($data, $css_code, $store_id) {
		...
		if (isset($data['main_color']) && !empty($data['main_color'])) {
			...
			$css .= ".oct-bottom-free-shipping-from .progress-bar, .oct-checkout-block ul.dropdown-address li:hover, .oct-checkout-block #button-confirm {background-color:". $data['main_color'] .";}".PHP_EOL;
		}
		...
		if (isset($data['main_link_color']) && !empty($data['main_link_color'])) {
			$css .= ".form-checkbox-group label a, .subcat-item:hover .subcat-item-title, .rm-content a, .blue-link, .blue-link:hover, .rm-category-wall-item:hover span, .rm-promotional-product-item:hover a, #oct-menu-ul .group-title a, .rm-last-reviews-item:hover .rm-last-reviews-item-title, .rm-module-item:hover .rm-module-title > a, .rm-module-item:hover .rm-module-title > a:visited, .rm-news-item:hover .rm-news-item-title, .rm-livesearch-list-item-name a, .rm-fixed-contact-item:hover, .category-landing-links-item-link span, .modal-content a, .rm-manufacturer-item a:hover span, .table td a, .rm-alert-text a, .rm-day-discounts-item:hover a, .rm-header-phones-list-item:hover, .rm-header-location-phones a .rm-footer-phones-link, .rm-footer-mail-link, .rm-footer-mail-link:hover, .collapse-value > button, .collapse-value > button:focus, .collapse-value > button:active, .collapse-value > button:active:focus, .rm-product-advantages-item:hover .rm-product-advantages-item-title, .rm-contact-location a, .rm-breadcrumb-item a, .rm-account-list-item:hover a, .rm-menu-list-item:hover > div:first-child .rm-menu-list-item-link, .rm-menu-list-item.active > div:first-child .rm-menu-list-item-link, .rm-footer-phones-link, .rm-footer-phones-link:hover, .rm-header-location-phones a, .oct-checkout-block a.blue-link, .oct-checkout-actions .form-check-label a, .ds-cart-recommended-products-item:hover .rm-module-title, .ds-cart-item-title:hover, .payment-method-check-item a {color:". $data['main_link_color'] .";}".PHP_EOL;
			$css .= ".rm-module-buttons button:hover {background-color:". $data['main_link_color'] .";border-color:". $data['main_link_color'] .";}".PHP_EOL;
		}
	...
	private function deleteOldGeneratedFiles($path) {
		if (!is_dir($path)) {
			return;
		}
    ...
    public function cacheDelete($redirect = true) {
	    if ($this->validateModification()) {
			...
			$minifyDir = rtrim(dirname(DIR_APPLICATION), '/') . '/catalog/view/theme/oct_remarket/stylesheet/';
			if (is_dir($minifyDir)) {
	...
	private function octInstallEvents() {
	...
	private function octUninstallEvents() {
		// Uninstall theme events
		$this->load->model('setting/event');

		$eventCodes = [
			'octemplates-remarket-user-account-sms-otp',
			'octemplates-remarket-order-sms-notify'
		];
	...
	public function install() {
		...
		$this->model_setting_setting->editSetting('theme_'. $this->oct_theme, [
			...
			'theme_'. $this->oct_theme .'_data_pr_reviews_limit' => 20,
			'theme_'. $this->oct_theme .'_data_bought_together_cron' => 0,
			'theme_'. $this->oct_theme .'_data_timer_enddate' => '',
