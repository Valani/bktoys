...
class ControllerOCTemplatesStickersOCTStickersSettings extends Controller {
	...
	public function index() {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			$this->load->controller('extension/theme/oct_remarket/cacheDelete', false);
		...
		if (isset($this->request->post['oct_stickers_cron_password'])) {
			$data['oct_stickers_cron_password'] = $this->request->post['oct_stickers_cron_password'];
		} else {
			$data['oct_stickers_cron_password'] = $this->config->get('oct_stickers_cron_password');
		}

		if (isset($data['oct_stickers_cron_password']) && !empty($data['oct_stickers_cron_password'])) {
            ...
            $data['cron_url'] = $site_link . "index.php?route=octemplates/stickers/cron_stickers&key=" . urlencode($data['oct_stickers_cron_password']);
        } else {
            $data['cron_url'] = '';
        }
