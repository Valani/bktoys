...
class ModelExtensionModuleOCFilterFilter extends Model {
  ...
  public function addFilter($data) {
    ...
    if (isset($data['filter_category'])) {
      ...
      foreach ($data['filter_category'] as $category_id => $name) {
        $this->db->query("INSERT INTO " . DB_PREFIX . "ocfilter_filter_to_category SET filter_id = '" . (int)$filter_id . "', source = " . (int)$this->ocfilter->params->source('default')->id() . ", category_id = '" . (int)$category_id . "', status = 1");
      }
    }
  ...
  public function editFilter($filter_key, $data) {
    ...
    $this->db->query("UPDATE " . DB_PREFIX . "ocfilter_filter_to_category SET status = '0' WHERE filter_id = '" . (int)$filter_id . "' AND source = " . (int)$source);
    ...
    if (isset($data['filter_category'])) {
      ...
      foreach ($data['filter_category'] as $category_id => $name) {
        $this->db->query("INSERT INTO " . DB_PREFIX . "ocfilter_filter_to_category SET filter_id = '" . (int)$filter_id . "', source = " . (int)$source . ", category_id = '" . (int)$category_id . "', status = '1' ON DUPLICATE KEY UPDATE status = VALUES(status)");
      }
    }
  ...
  public function getFilters($data = []) {
    ...
    if (isset($data['filter_category_id'])) {
      if (is_array($data['filter_category_id'])) {
        ...
        $sql .= " AND f2c.category_id IN('" . implode("','", array_map('intval', array_unique($data['filter_category_id']))) . "') AND f2c.status = '1'";
      } else {
        if ($data['filter_category_id'] > 0) { // Regular search
          $sql .= " AND (f2c.category_id = '" . (int)$data['filter_category_id'] . "' OR f2c.category_id = '0') AND f2c.status = '1'"; // Include filters with all categories
        } else if ($data['filter_category_id'] < 0) {
          $sql .= " AND f2c.category_id IS NULL"; // Not has categories
        } else {
          $sql .= " AND f2c.category_id = '0' AND f2c.status = '1'"; // Only with all categories
        }
      }
    }
...
/*     $this->writeLog('[Optimize]');
  ...
  private function setCopyFilterCategory($data) {
    ...
    $this->db->query("INSERT IGNORE INTO " . DB_PREFIX . "ocfilter_filter_to_category (filter_id, source, category_id, status) SELECT fv2p.filter_id, fv2p.source, p2c.category_id, '1' FROM " . DB_PREFIX . "ocfilter_filter_value_to_product fv2p LEFT JOIN " . DB_PREFIX . "product_to_category p2c ON (p2c.product_id = fv2p.product_id) WHERE p2c.category_id != '0' AND fv2p.source != " . (int)$this->ocfilter->params->source('default')->id() . " GROUP BY fv2p.filter_id, fv2p.source, p2c.category_id");
  }
