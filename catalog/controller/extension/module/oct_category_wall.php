...
class ControllerExtensionModuleOctCategoryWall extends Controller {
    public function index($setting) {
        ...
        if (isset($setting['module_categories']) && $setting['module_categories']) {
	        ...
	        $cache_result = $this->cache->get('octemplates.category_wall.' . (int)$this->config->get('config_language_id') . '.' . (int)$this->config->get('config_store_id') . '.' . $this->config->get('config_customer_group_id') . '.' . md5(json_encode($setting)) . '.' . $oct_webP);
            $categories_wall = is_array($cache_result) ? $cache_result : [];
