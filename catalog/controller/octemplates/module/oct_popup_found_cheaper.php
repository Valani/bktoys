...
class ControllerOCTemplatesModuleOctPopupFoundCheaper extends Controller {
	    ...
	    if ($this->config->get('oct_popup_found_cheaper_status') && isset($this->request->server['HTTP_X_REQUESTED_WITH']) && !empty($this->request->server['HTTP_X_REQUESTED_WITH']) && strtolower($this->request->server['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') {
	        ...
	        if ($this->validate()) {
	            ...
	            if (isset($oct_popup_found_cheaper_data['notify_status']) && $oct_popup_found_cheaper_data['notify_status']) {
					...
					$template_code = 'oct_popup_found_cheaper';
					...
					if (isset($oct_sms_settings['templates'][$template_code]['status']) && $oct_sms_settings['templates'][$template_code]['status']) {
						...
						if (!empty($sms_template)) {
							$replace = array(
								...
								'[link]' => isset($this->request->post['link']) ? htmlspecialchars($this->request->post['link']) : '',
								'[product_link]' => isset($this->request->post['pid']) ? htmlspecialchars($this->request->post['pid']) : '',
    ...
    protected function validate() {
		...
		if (isset($this->request->post['telephone'])) {
			...
			$this->request->post['telephone'] = preg_replace("/[^0-9\(\)\-\_\+ ]/", '', $telephone);
			$mask = $oct_popup_found_cheaper_data['mask'] ?? '';
			$telephone_setting = $oct_popup_found_cheaper_data['telephone'] ?? '';
