...
class ControllerOCTemplatesModuleOctPopupPurchase extends Controller {
	...
	public function index() {
		if ($this->config->get('oct_popup_purchase_status') && $this->config->get('oct_popup_purchase_data') && $this->config->get('config_checkout_guest') && isset($this->request->server['HTTP_X_REQUESTED_WITH']) && !empty($this->request->server['HTTP_X_REQUESTED_WITH']) && strtolower($this->request->server['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') {
            ...
            if((isset($oct_analytics_google_ecommerce) && $oct_analytics_google_ecommerce)) {
	...
	public function makeOrder() {
		...
		$is_google_bot = (strpos($user_agent, "Google") !== false);

		if (!$is_google_bot && ($this->config->get('oct_popup_purchase_status') || $this->config->get('oct_popup_purchase_byoneclick_status')) && $this->config->get('config_checkout_guest') && isset($this->request->server['HTTP_X_REQUESTED_WITH']) && !empty($this->request->server['HTTP_X_REQUESTED_WITH']) && strtolower($this->request->server['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest' && (isset($this->request->post['product_id']) && !empty($this->request->post['product_id']))) {
			$this->load->language('octemplates/module/oct_popup_purchase');
			...
			if ($this->validate()) {	
				...
				$checkTotal = $this->checkMinimumOrderAmount();

				if (isset($checkTotal['error_total'])) {
					$json['error']['minimum_order_amount'] = $checkTotal['error_total'];
	...
	private function checkMinimumOrderAmount() {
		...
		if (isset($oct_popup_purchase_data['minimum_order_amount']) && !empty($oct_popup_purchase_data['minimum_order_amount'])) {
			$minimumOrderAmount = (float) $oct_popup_purchase_data['minimum_order_amount'];
		}

		if ($total < $minimumOrderAmount) {
			$error['error_total'] = sprintf($this->language->get('error_minimum_order_amount'), $this->currency->format($this->tax->calculate($minimumOrderAmount, $this->config->get('config_tax'), $this->config->get('config_tax_class_id')), $this->session->data['currency']));
		}
	...
	protected function validate() {
		...
		if (isset($this->request->post['telephone'])) {
			...
			$telephone_setting = $oct_popup_purchase_data['telephone'] ?? '';
			$theme_data = $this->config->get('theme_oct_remarket_data');
