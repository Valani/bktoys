...
class ModelExtensionModuleOCFilter extends Model {
  ...
  protected function getCategoryFilters($data = []) {
    ...
    if (isset($data['filter_category_id'])) {
      if ($this->ocfilter->config('category_visibility') == 'parent') {
        $sql .= " AND (cp.path_id = '" . (int)$data['filter_category_id'] . "' OR f2c.category_id = '0') AND f2c.status = '1'";
      } else {
        $sql .= " AND (f2c.category_id = '" . (int)$data['filter_category_id'] . "' OR f2c.category_id = '0') AND f2c.status = '1'";
      }
    }
