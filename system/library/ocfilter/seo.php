...
class Seo extends Factory {
  ...
  public function isSeoUrlEnabled() {
    ...
    if (isset($this->opencart->request->server['QUERY_STRING']) && (substr($this->opencart->request->server['QUERY_STRING'], 0, 8) != '_route_=') || (false !== strpos($this->opencart->request->server['QUERY_STRING'], 'index.php'))) {
      $seo_url = false;
    }
  ...
  public function isNoIndex() {
    $noindex = ($this->getParams() && !$this->getPageInfo());

    if (!$noindex && $this->getManufacturerId() && $this->placement->isCategory()) {
      // TODO: Move to the admin panel setting $noindex = true;
    }
  ...
  public function getProductPages() {
    ...
    if ($query->num_rows) {
      ...
      // Is has discount?
      $query = $this->opencart->db->query("SELECT * FROM " . DB_PREFIX . "product_special WHERE product_id = '" . (int)$this->getProductId() . "' AND customer_group_id = '" . (int)$this->opencart->config->get('config_customer_group_id') . "' AND ((date_start = '0000-00-00' OR date_start < '" . $this->opencart->db->escape(date('Y-m-d')) . "') AND (date_end = '0000-00-00' OR date_end > '" . $this->opencart->db->escape(date('Y-m-d')) . "')) LIMIT 1");

      if (!$query->num_rows) {
        $query = $this->opencart->db->query("SELECT * FROM " . DB_PREFIX . "product_discount WHERE product_id = '" . (int)$this->getProductId() . "' AND customer_group_id = '" . (int)$this->opencart->config->get('config_customer_group_id') . "' AND quantity = '1' AND ((date_start = '0000-00-00' OR date_start < '" . $this->opencart->db->escape(date('Y-m-d')) . "') AND (date_end = '0000-00-00' OR date_end > '" . $this->opencart->db->escape(date('Y-m-d')) . "')) LIMIT 1");
      }

      if ($query->num_rows) {
        $product_params[$this->params->special('discount')->key()] = [ 1 ];
      }
    }
